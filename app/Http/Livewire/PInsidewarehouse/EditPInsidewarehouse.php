...
namespace App\Http\Livewire\PInsidewarehouse;
...
class EditPInsidewarehouse extends Component
{
    use WithPagination;
    ...
    public function save_plan()
    {
        ...
        } elseif ($this->Plan_TYPE_ID == 30) {
            $alertType = 'alert_outside';
        } elseif ($this->Plan_TYPE_ID == 31) {
            $alertType = 'alert_inside';
        }
    ...
    public function selectRow($PRODCT_LINK_NO)
    {
        $VW_EQUIPMENT = DB::table('VW_PRODUCT')->where('PRODCT_LINK_NO', $PRODCT_LINK_NO)
            ->first();

        DB::table('EQUIPMENT_LIST')->insert([
            'Equip_SERIAL_NUMBER' => $VW_EQUIPMENT->PRODCT_ID,
            'Equip_NAME' => $VW_EQUIPMENT->PRODCT_NAME,
            'Equip_TCHN_LOCAT_ID' => $VW_EQUIPMENT->IC_LOCAT_ID,
            'Equip_STS_ID' => $VW_EQUIPMENT->PRODCT_IOSTK_FLAG,
            'Equip_PRICE' => $VW_EQUIPMENT->PRODCT_MIN_COSTRATE,
            'Equip_LINK_NO' => $VW_EQUIPMENT->PRODCT_LINK_NO,
            'Equip_USED' => 0,
    ...
    public function render()
    {
        ...
        if ($this->search_EQUIP) {
            $this->showTable = true;
            $VW_EQUIPMENT = DB::table('VW_PRODUCT')
                ->where(function ($query) {
                    $query->where('PRODCT_NAME', 'like', '%' . $this->search_EQUIP . '%')
                        ->orWhere('PRODCT_ID', 'like', '%' . $this->search_EQUIP . '%');
                })
                ->where('IC_LOCAT_ID', Auth::user()->deptId)  // เพิ่มเงื่อนไขสำหรับ deptId
                ->whereIn('PRODCT_IOSTK_FLAG', ['A', 'I'])  // เพิ่มเงื่อนไขสำหรับ PRODCT_IOSTK_FLAG
                ->orderBy('PRODCT_ID', 'asc')
        ...
        return view(
            'livewire.PInsidewarehouse.edit',
            [
