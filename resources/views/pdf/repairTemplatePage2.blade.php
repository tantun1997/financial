    ...
    <table width="100%" class="tableHeader" cellspacing="0" cellpadding="0">
        ...
        <tr>
            <td>
                ...
                งานจัดซื้อพัสดุ จำนวน {{ $Total_Used }} รายการ ตามแผน {{ $Plan_NAME }} ประจำปี {{ $Plan_YEAR }}
            </td>
        </tr>
        <tr>
    ...
    <table width="100%" class="tableList" style="margin-bottom: 10px;" cellspacing="0" cellpadding="0">
        ...
        @foreach ($List_Equip as $item)
            <tr>
                ...
                <td style="text-align: center;">{{ $item->Equip_SERIAL_NUMBER }}</td>
                <td style="text-align: left;">{{ $item->Equip_NAME }}</td>
                <td style="text-align: center;">{{ $item->Equip_AGE }} ปี</td>
                <td style="text-align: right;">{{ number_format($item->Equip_PRICE, 2) }}</td>
                <td style="text-align: right;">{{ number_format($item->Equip_CURRENT_PRICE, 2) }}</td>
            </tr>
        @endforeach
