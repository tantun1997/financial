<div class="container-fluid px-4">
    <h3 class="mt-3 mb-3"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24"
        ...
        </svg> รายละเอียดแผนเพิ่มศักยภาพ</h3>
        ...
        <div class="col-lg-4">
            @if ($edit_plan)
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                    <div>
                        <a class="btn btn-outline-primary" href="{{ route('potential_plan') }}">ย้อนกลับ</a>
                    </div>
                    <div style="display: flex; align-items: center;">
                        <a class="btn btn-outline-danger"
                            href="http://192.168.2.142/potential_plan/detail?id={{ $Plan_ID }}">ยกเลิก</a>
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 10px;">
                <div style="display: flex; align-items: center;">
                    <button wire:click="addRow({{ $Plan_ID }})" class="btn btn-outline-success">
                        <i class="fa-solid fa-arrow-up-from-bracket"></i> เพิ่มการเพิ่มศักยภาพ
                    </button>
    ...
    <script>
        function generatePdf(id) {
            ...
            if ({{ $EQUIPMENT_PLAN->Plan_LEVEL }} == 2) {
                ...
                // แสดง Alert ให้ผู้ใช้เลือก
                toastr.error('เพิ่มการเพิ่มศักยภาพก่อน');
            } else if ({{ $EQUIPMENT_PLAN->Checked_use }} == 0) {

                // แสดง Alert ให้ผู้ใช้เลือก
                toastr.error('เลือกรายการเพิ่มศักยภาพ');
            } else {

                // เปิดหน้าต่างใหม่เพื่อสร้าง PDF
                window.open('/PotentialPDF/' + id, '_blank');
